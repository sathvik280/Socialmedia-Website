import React, { useState, useEffect } from 'react';
import { useSelector, useDispatch } from 'react-redux';
import axios, { AxiosResponse } from 'axios';

import { ImSpinner8 } from 'react-icons/im';

import { Post, Store, ResponseDataFeedPosts } from '../types/interface';

import PostF from './PostF';

import { setFeedPosts, setIsFeedPostsFetched } from '../features/slice/homeSlice';

const FeedPosts: React.FC = (): JSX.Element => {
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [isError, setIsError] = useState<boolean>(false);

    const dispatch = useDispatch();

    const { feedPosts, isFeedPostsFetched }: {
        feedPosts: Array<Post>, 
        isFeedPostsFetched: boolean 
    } = useSelector( (store: Store) => store.home);

    const { token }: {
        token: string
    } = useSelector( (store: Store) => store.auth);

    const fetchPosts: Function = async (url: string): Promise<void> => {
        try 
        {
            const response: AxiosResponse<ResponseDataFeedPosts> = await axios.get(url, {
                headers: {
                    'authorization': token
                }
            });
            const data: ResponseDataFeedPosts = await response.data;

            const { posts }: {
                posts: Array<Post>
            } = data;

            dispatch(setFeedPosts(posts));
            dispatch(setIsFeedPostsFetched());
            
            setIsLoading(false);
        }

        catch (error: any)
        {
            console.log(error);

            if (error.response === undefined)
            {
                console.log('Server down, please try again');
            }

            else 
            {
                console.log('Server error, please try again');
            }

            setIsError(true);
            setIsLoading(false);
        }
    };

    useEffect( () => {
        if (!isFeedPostsFetched)
        {
            setIsLoading(true);
            fetchPosts('https://socialmedia-server-qq3r.onrender.com/post');
        }
    }, []);

    return (
        <div className="grid grid-cols-1 gap-y-8 w-[90%] max-w-[550px] mx-auto">
            {
                isLoading 
                ? 
                <div className="w-full mt-20 flex items-center justify-center">
                    <ImSpinner8 className="text-4xl text-[#A3A3A3] animate-spin"/>
                </div> 
                :
                isError
                ?
                <div 
                    className="bg-white rounded-lg flex justify-center items-center py-6"
                    style={ {boxShadow: "0px 0px 8px 0px lightgrey"} }
                >
                    <div className="text-[18px] text-[#666666] px-2 text-center">
                        Error, please refresh the page
                    </div>
                </div>
                :
                feedPosts.map( (post: Post): JSX.Element => {
                    return (
                        <div key={post._id}>
                            <PostF post={post} />
                        </div>
                    );
                })
            }
        </div>
    );
};

export default FeedPosts;